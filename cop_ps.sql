CREATE OR REPLACE FUNCTION cop_ps(geometry, geometry)
  RETURNS boolean AS
$BODY$
DECLARE
g1 geometry;
...
IF (GEOMETRYTYPE(g1) = 'POLYGON' AND GEOMETRYTYPE(g2) = 'LINESTRING' AND ST_NUMPOINTS(g2) = 2) THEN
   IF ST_NDIMS(g1) <> 3 OR ST_NDIMS(g2) <> 3 THEN
      RAISE NOTICE 'Input geometries are not 3D';
      RETURN false;
   END IF;
   RAISE NOTICE 'COPLANAR between a patch and a segment in 3D';
   pa := g1;
   s := g2;
   RAISE NOTICE 'Patch: %', ST_ASEWKT(pa);
   RAISE NOTICE 'Segment: %', ST_ASEWKT(s);
   
   -- Testing if pa is planar
   pt1 := ST_PointN(ST_EXTERIORRING(pa),1);
   --RAISE NOTICE 'Pt1: %', ST_ASEWKT(pt1);
   pt2 := ST_PointN(ST_EXTERIORRING(pa),2);
   --RAISE NOTICE 'Pt2: %', ST_ASEWKT(pt2);
   pt3 := ST_PointN(ST_EXTERIORRING(pa),3);
   ...
   FOR pt IN SELECT geom FROM ST_DUMPPOINTS(pa) LOOP
	    ...
	    IF abs((ST_X(pt)*A1 + ST_Y(pt)*A2 + ST_Z(pt)*A3) - A) < prec THEN     
		   CONTINUE;
	    ELSE
		   RAISE NOTICE 'Input 3D polygon is not planar';
		   RETURN false;
	    END IF;					   
   END LOOP;
   pt := ST_PointN(s,1);
   --RAISE NOTICE 'Pt1-s: %', ST_ASEWKT(pt);								 
   IF abs((ST_X(pt)*A1 + ST_Y(pt)*A2 + ST_Z(pt)*A3) - A) > prec THEN  
	  RETURN false;
   END IF;
   pt := ST_PointN(s,2);
   --RAISE NOTICE 'Pt2-s: %', ST_ASEWKT(pt);	
   IF abs((ST_X(pt)*A1 + ST_Y(pt)*A2 + ST_Z(pt)*A3) - A) > prec THEN  
	  RETURN false;
   END IF;
   RETURN true;
ELSE								 
   RAISE NOTICE 'Input geometries are not a patch and a segment';
   RETURN false;
END IF;   
END;
$BODY$
